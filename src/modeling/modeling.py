...
class OursBaseModel(BertPreTrainedModel):
    """
...
class OursForVideoTextRetrieval(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(OursForVideoTextRetrieval, self).__init__(config)
        self.config = config

        self.bert = OursBaseModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

...
def setup_e2e_optimizer(model, opts):
    ...
    transformer_emb_param_optimizer = [
        (n, p) for n, p in parameters
        if "transformer" in n  and "embeddings" in n and p.requires_grad]
    trasformer_emb_grouped_parameters = build_e2e_optimizer_w_lr_mul(
        transformer_emb_param_optimizer,
        opts.text_embedding_learning_rate, opts.weight_decay,
        lr_mul=opts.transformer_lr_mul,
        lr_mul_prefix=opts.transformer_lr_mul_prefix)

    transformer_param_optimizer = [
        (n, p) for n, p in parameters
        if "transformer" in n  and "embeddings" not in n and p.requires_grad]
    ...
    optimizer_grouped_parameters = []
    optimizer_grouped_parameters.extend(trasformer_emb_grouped_parameters)
